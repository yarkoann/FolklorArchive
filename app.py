...
def api_delete_recording(rid):
    try:
        ...
        deleted = delete_recording_data(rid)

        # Также удаляем из менеджера
        try:
        ...
        traceback.print_exc()
        # Даже при ошибке, если запись была удалена, возвращаем успех
        return jsonify({'status': 'success', 'message': 'Запись удалена'})
    ...
    for r in recs:
        for mf in r.get('media_files', []):
            if mf.get('stored_name') == safe:
                original = mf.get('original_name', safe);
                break
...
def api_vocab_add_item(cat):
    try:
        ...
        if cat == 'ethnos':
            new_item.update({'alternative_names': d.get('alternative_names', []), 'region': d.get('region', '')})
        elif cat == 'genres':
            new_item.update({'local_terms': d.get('local_terms', []), 'ritual': d.get('ritual', False)})
        elif cat == 'locations':
            new_item['type'] = d.get('type', 'other')
        elif cat == 'instruments':
            new_item.update({'type': d.get('type', 'other'), 'local_names': d.get('local_names', [])})
...
# ── performers from recordings ──────────────────────────────────────

@app.route('/api/performers_all')
def api_performers_all():
    """Все исполнители из всех записей с контекстом"""
    try:
        _, recs = load_data()
        result = []
        seen = {}
        for r in recs:
            for performer in r.get('performers', []):
                if not performer.get('name'): continue
                name = performer['name'].strip()
                key = name.lower()
                if key not in seen:
                    seen[key] = {
                        'name': name,
                        'ethnos': performer.get('ethnos'),
                        'recordings': [],
                        'performance_forms': [],
                        'description': '',
                        'photo_url': ''
                    }
                if r['id'] not in [rec['id'] for rec in seen[key]['recordings']]:
                    seen[key]['recordings'].append({'id': r['id'], 'title': r.get('title', '')})
                if performer.get('performance_form') and performer['performance_form'] not in seen[key][
                    'performance_forms']:
                    seen[key]['performance_forms'].append(performer['performance_form'])
        # Загружаем сохранённые описания и фото
        perf_meta = manager.vocabulary.get('performers_meta', {})
        for key, item in seen.items():
            meta = perf_meta.get(key, {})
            item['description'] = meta.get('description', '')
            item['photo_url'] = meta.get('photo_url', '')
            result.append(item)
...
@app.route('/api/performer_desc/update', methods=['POST'])
def api_performer_desc_update():
    """Сохранение описания и фото для исполнителя"""
    try:
        d = request.json
        name = d.get('name', '').strip()
        ...
        if not name:
            return jsonify({'status': 'error', 'message': 'Имя исполнителя не указано'}), 400
        fv = manager.vocabulary
        fv.setdefault('performers_meta', {})
        key = name.lower()
        fv['performers_meta'][key] = {
            'name': name,
